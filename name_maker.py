...
race = {
    ...
    "half-orc": {
        ...
        "clan": [
            "Bloodgrin", "Stonemaul", "Thunderfist", "Graveltongue", "Ragetotem",
            "Ironmarch", "Bonegrinder", "Doomclaw", "Earthshaker", "Cragbasher",
            "Frostvein", "Netherfang", "Tuskthorn", "Stormbellow", "Emberblade",
            "Skullsmasher", "Broadshield", "Grimstalker", "Wolfcaller", "Stormhowler",
            "Earthsplitter", "Foe-hammer", "Shadowhunter", "Peakwarden", "Fangscar"
        ]
    },
...
def get_name():
    for i in range(len(sys.argv)):
        if len(sys.argv) >= 1 and sys.argv[i] not in available_races:
            ...
            if sys.argv[i] in available_races:
                name = get_specific_name()
    return name


def get_specific_name_CLI():
    for i in range(len(sys.argv)):
        if sys.argv[i] in available_races:
            chosen_race = sys.argv[i].lower()
            ...
            elif chosen_race == "halfelf":
                name = get_half_elf()

            elif chosen_race == "halforc":
                name = get_half_orc()

            elif chosen_race == "tiefling":
                name = get_tiefling()
...
def get_name_UI():
    race_functions = {
        "dragonborn": get_dragonborn,
        "dwarf": get_dwarf,
        "elf": get_elf,
        "halfling": get_halfling,
        "gnome": get_gnome,
        "human": get_human,
        "half elf": get_half_elf,
        "half orc": get_half_orc,
        "tiefling": get_tiefling,
    }

    race_keys = sorted(list(race_functions.keys()))

    while True:
        print("What would you like this character's race to be?")
        print()
        for i, race in enumerate(race_keys):
            print(f"{race.title():<15}", end="")
            if (i + 1) % 3 == 0 or (i + 1) == len(race_keys):
                print()
        print()
        chosen_race = input("Answer: ").lower()

        if chosen_race in race_keys:
            name = race_functions[chosen_race]()
            return name
        else:
            unsupported_error(chosen_race)
            continue


def supported_races():
    for i in range(len(available_races)):
        print(f"{available_races[i]:<15}", end="")
        if (i + 1) % 3 == 0:
            print()
    if len(available_races) % 3 != 0:
        print()


def unsupported_error(str):
    print("Your entry is either not currently supported, or there was a typo.")
...
def get_random_name():
    ...
    # Dragonborn
    NPC_name_list.append(get_dragonborn())

    # Dwarf
    NPC_name_list.append(get_dwarf())

    # Elf
    NPC_name_list.append(get_elf())

    # Gnome
    NPC_name_list.append(get_gnome())

    # Halfling
    NPC_name_list.append(get_halfling())

    # Human
    NPC_name_list.append(get_human())

    # Half-Elf
    NPC_name_list.append(get_half_elf())

    # Half-Orc
    NPC_name_list.append(get_half_orc())

    # Tiefling
    NPC_name_list.append(get_tiefling())

    NPC_name = random.choice(NPC_name_list)
    return NPC_name
...
def get_dragonborn():
    rand_age = random.randint(16, 85)
    for year in range(rand_age):
        if year in range(80, 85):
            age = (f"an {year} year old")
        else:
            age = (f"a {year} year old")

    color = str(random.choice(list(race["dragonborn"]["color"])))
    NPC_sex = random.choice(sex)

    first_name = str(random.choice(list(race["dragonborn"][NPC_sex])))
    last_name = str(random.choice(list(race["dragonborn"]["clan"])))

    return (f"Before you is {age}, {color} dragonborn named {first_name} {last_name}.")
...
def get_dwarf():
    rand_age = random.randint(35, 365)
    for year in range(rand_age):
        if year in range(80, 85):
            age = (f"an {year} year old")
        else:
    ...
    subrace = str(random.choice(list(race["dwarf"]["subrace"])))

    NPC_sex = random.choice(sex)
    first_name = str(random.choice(list(race["dwarf"][NPC_sex])))
    last_name = str(random.choice(list(race["dwarf"]["clan"])))

    return (f"Before you is {age}, {subrace} dwarf named {first_name} {last_name}.")
...
def get_elf():
    rand_age = random.randint(25, 785)
    for year in range(rand_age):
        if year in range(80, 85):
            age = (f"an {year} year old")
        else:
    ...
    subrace = str(random.choice(list(race["elf"]["subrace"])))

    if rand_age <= 100:
        first_name = str(get_child_elf_first_name())
    elif rand_age >= 101:
        first_name = str(get_adult_elf_first_name())
    last_name = str(random.choice(list(race["elf"]["clan"])))

    return (f"Before you is {age}, {subrace} elf named {first_name} {last_name}.")


def get_child_elf_first_name():
    first_name = random.choice(list(race["elf"]["child"]))
    return first_name


def get_adult_elf_first_name():
    elf_sex = ["male", "female"]
    NPC_sex = random.choice(elf_sex)
    first_name = random.choice(list(race["elf"][NPC_sex]))
    return first_name
...
def get_gnome():
    rand_age = random.randint(30, 515)
    for year in range(rand_age):
        if year in range(80, 85):
            age = (f"an {year} year old")
        else:
    ...
    subrace = random.choice(list(race["gnome"]["subrace"]))

    NPC_sex = random.choice(sex)

    first_name = str(random.choice(list(race["gnome"][NPC_sex])))
    clan_name = str(random.choice(list(race["gnome"]["clan"])))

    return (f"Before you is {age}, {subrace} gnome named {first_name} {clan_name}.")
...
def get_halfling():
    rand_age = random.randint(19, 265)
    for year in range(rand_age):
        if year in range(80, 85):
            age = (f"an {year} year old")
        else:
    ...
    subrace = str(random.choice(list(race["halfling"]["subrace"])))

    NPC_sex = random.choice(sex)
    first_name = str(random.choice(list(race["halfling"][NPC_sex])))
    last_name = str(random.choice(list(race["halfling"]["clan"])))

    return (f"Before you is {age}, {subrace} halfling named {first_name} {last_name}.")
...
def get_human():
    age = (f"a {random.randint(16, 66)} year old")
    ...
    first_name = str(random.choice(list(race["human"]["culture"][chosen_culture][NPC_sex])))
    last_name = str(random.choice(list(race["human"]["culture"][chosen_culture]["surname"])))

    return (f"Before you is {age} human named {first_name} {last_name}.")
...
def get_half_elf():
    rand_age = random.randint(18, 205)
    for year in range(rand_age):
        if year in range(80, 85):
            age = (f"an {year} year old")
        else:
    ...
    background = ["human", "elf"]
    NPC_background = random.choice(background)

    if NPC_background == "human":
        NPC_sex = random.choice(sex)
        cultures = list(race["human"]["culture"].keys())
        chosen_culture = random.choice(cultures)
    ...
    elif NPC_background == "elf" and rand_age <= 100:
        first_name = random.choice(list(race["elf"]["child"]))

    elif NPC_background == "elf" and rand_age >= 101:
        elf_sex = ["male", "female"]
        NPC_sex = random.choice(elf_sex)
        first_name = random.choice(list(race["elf"][NPC_sex]))

    last_name = str(get_half_elf_last_name())

    return (f"Before you is {age} half-elf named {first_name} {last_name}.")


def get_half_elf_last_name():
    background = ["human", "elf"]
    NPC_background = random.choice(background)
    ...
    if NPC_background == "human":
        cultures = list(race["human"]["culture"].keys())
        chosen_culture = random.choice(cultures)
        names = list(race["human"]["culture"][chosen_culture]["surname"])
        last_name = random.choice(names)

    elif NPC_background == "elf":
        last_name = random.choice(list(race["elf"]["clan"]))
...
def get_half_orc():
    background = ["human", "orc"]
    NPC_background = random.choice(background)
    age = (f"a {random.randint(13, 60)} year old")
    ...
    elif NPC_background == "orc":
        NPC_sex = random.choice(sex)
        first_name = random.choice(list(race["half-orc"][NPC_sex]))

    # Last Name
    if NPC_background == "orc":
        cultures = list(race["human"]["culture"].keys())
        chosen_culture = random.choice(cultures)
        surnames = list(race["human"]["culture"][chosen_culture]["surname"])
        last_name = random.choice(surnames)

    elif NPC_background == "human":
        last_name = random.choice(list(race["half-orc"]["clan"]))

    return (f"Before you is {age} half-orc named {first_name} {last_name}.")
...
def get_tiefling():
    rand_age = random.randint(17, 105)
    for year in range(rand_age):
        if year in range(80, 85):
            age = (f"an {year} year old")
        else:
    ...
    NPC_sex = random.choice(sex)
    first_name = str(random.choice(list(race["tiefling"][NPC_sex])))
    return (f"Before you is {age} tiefling named {first_name}.")
